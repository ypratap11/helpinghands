...
- **Recording past work (historical backfill).** The group has years of prior help recorded only in WhatsApp; they must be able to add a past cause without itemising payments made long ago. The create form includes an **optional** amount field — label it clearly, e.g. "Total already given (for a past cause)" with a hint that it's only for recording work done before this system. When present on create, `saveCaseAction` creates the case and then, in the same flow, records a **single disbursement** for that amount with `paidOn = occurredOn`, `mode = OTHER`, and `note = "Recorded as a past total"`, via `createDisbursement` (so it's audited and counts toward the case's disbursed total and the public impact figures like any other). `occurredOn` may be any past date (the date input already allows it; server-side still round-trip-validates it's a real date). If the amount field is empty, no disbursement is created — an ordinary new case. Editing an existing case must NOT re-trigger this (it's create-only) — guard on the absence of an id. Add a test: creating a case with a "total already given" of `"50,000"` produces exactly one disbursement of `5000000` paise dated to `occurredOn`, and creating without it produces zero disbursements.
...
Requirements:
- The homepage keeps the warm identity (aura, Fraunces, forest/marigold, `.mark`, `.lift`) and adds, below the hero, a **live impact band**: three (or four, if balance is public) big figures from `publicImpact()` — "Raised", "Given out", "People helped", and "In hand" only when `balancePaise !== null`. Use `Money … compact` and `font-display`; label each plainly. These figures include historical/backfilled cases and contributions automatically (they are ordinary disbursements/contributions), so a group that backfills its WhatsApp history sees its full track record here. If everything is zero (no data yet), show an honest, warm empty state ("The ledger is just getting started") rather than a wall of ₹0.
